...
class GMM_TPS(nn.Module):
    def __init__(self, input_nc, output_nc=3):
        super(GMM_TPS, self).__init__()
        self.stn = TPS()
        nl = nn.InstanceNorm2d
        self.conv1 = nn.Sequential(*[nn.Conv2d(input_nc, 64, kernel_size=3, stride=1, padding=1), nl(64), nn.ReLU()])
        self.pool1 = nn.MaxPool2d(kernel_size=(2, 2))
...
# ------------------------------------------------ encoder-decoder split --------------------------------------------------------
    ...
    def forward(self, clothes, torso, pose):
        input, rx,ry,cx,cy = self.stn(clothes, torso)
        conv1 = self.conv1(torch.cat([input.detach(), torso.detach(), pose.detach()], 1))
        pool1 = self.pool1(conv1)
        conv2 = self.conv2(pool1)
        pool2 = self.pool2(conv2)
        conv3 = self.conv3(pool2)
        ...
        # Encoder has 7 convolution layers
        # Decoder has 10 convolution layers
        return conv9, input, rx,ry,cx,cy

class GMM_OLD(nn.Module):
    def __init__(self, input_nc, output_nc=3):
        super(GMM_OLD, self).__init__()
        self.stn = Net()
        nl = nn.InstanceNorm2d
        self.conv1 = nn.Sequential(*[nn.Conv2d(input_nc, 64, kernel_size=3, stride=1, padding=1), nl(64), nn.ReLU(),
                                     nn.Conv2d(64, 64, kernel_size=3, stride=1, padding=1), nl(64), nn.ReLU()])
...
class GMM_OLD_TPS(nn.Module):
    def __init__(self, input_nc, output_nc=3):
        super(GMM_OLD_TPS, self).__init__()
        self.stn = TPS()
        nl = nn.InstanceNorm2d
        self.conv1 = nn.Sequential(*[nn.Conv2d(input_nc, 64, kernel_size=3, stride=1, padding=1), nl(64), nn.ReLU(),
                                     nn.Conv2d(64, 64, kernel_size=3, stride=1, padding=1), nl(64), nn.ReLU()])
